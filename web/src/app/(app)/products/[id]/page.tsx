  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start gap-4">
        <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100 text-3xl">{product.imageEmoji}</div>
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">{product.name}</h1>
          <p className="mt-1 text-sm text-slate-500">{product.brand} · {product.category} · {product.priceLabel}</p>
        </div>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <Card>
          ...
          <div className="divide-y divide-slate-100">
            {matches.map((inf) => (
              <Link key={inf.id} href={`/influencers/${inf.id}`} className="flex items-center justify-between gap-3 px-5 py-3 hover:bg-slate-50">
                <div>
                  <div className="text-sm font-medium text-slate-900">{inf.name}</div>
                  <div className="text-xs text-slate-500">{inf.city}</div>
                </div>
